...
void checkForDuplicates(const GetoptWrapper::DescOptions &opts)
{
    ...
    {
        if (std::find_if(opts.begin(), opts.end(), [&systemOpt](const GetoptWrapper::DescOption &val) {
                return systemOpt == val;
            }) != opts.end())
        {
            errcnt++;
    ...
    if (errcnt)
    {
        throw std::invalid_argument(stdutils::string_format("%s(): duplicates found - %zu", __func__, errcnt));
    }
}
...
GetoptWrapper::ProcessStatus GetoptWrapper::process(int argc, char **argv, int *index, ParsedOption &parsed) const
{
    ...
    std::transform(_options.begin(), _options.end(), std::back_inserter(longopts), [](const DescOption & val){return val.rawOption();});
