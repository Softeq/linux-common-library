...
namespace
{
const std::map<std::string, softeq::common::net::Method> methodTypeFromString{
    {MHD_HTTP_METHOD_GET, softeq::common::net::Method::GET},
    {MHD_HTTP_METHOD_POST, softeq::common::net::Method::POST},
    {MHD_HTTP_METHOD_PUT, softeq::common::net::Method::PUT},
    {MHD_HTTP_METHOD_DELETE, softeq::common::net::Method::DELETE},
    {MHD_HTTP_METHOD_OPTIONS, softeq::common::net::Method::OPTIONS}};
}
...
bool HttpServerImpl::start()
{
    ...
    {
#ifdef HTTPS_ENABLED
        serverStarted = startServerHttps();
#else
        LOGE(LOG_DOMAIN, "SSL is not available");
#endif
    }
    else
    {
        serverStarted = startServerHttp();
    }
    ...
    _cron->addJob("Check HTTP sessions expiration", "* * * * * *", true,
                  std::bind(&HttpServerImpl::checkSessionsExpiration, this));
    _cron->start();
...
#ifdef HTTPS_ENABLED
bool HttpServerImpl::startServerHttps()
{
    assert(_server == nullptr);

    if (!fsutils::readBinaryFileIntoBuffer(_settings.certFilePath, _certBuffer))
    {
        LOGE(LOG_DOMAIN, "Couldn't load SSL certificate from file %s.", _settings.certFilePath.c_str());
    }

    if (!fsutils::readBinaryFileIntoBuffer(_settings.keyFilePath, _keyBuffer))
    {
        LOGE(LOG_DOMAIN, "Couldn't load SSL key from file %s.", _settings.keyFilePath.c_str());
    }

    if (_certBuffer && _keyBuffer)
    {
        _server = MHD_start_daemon(
            MHD_USE_THREAD_PER_CONNECTION | MHD_USE_INTERNAL_POLLING_THREAD | MHD_USE_DEBUG |
                MHD_USE_PIPE_FOR_SHUTDOWN | MHD_USE_SSL,
            _settings.port, nullptr, nullptr, &mhdEventHandler, this, MHD_OPTION_EXTERNAL_LOGGER, mhdLogger, nullptr,
            MHD_OPTION_NOTIFY_COMPLETED, mhdRequestCompleted, nullptr, MHD_OPTION_CONNECTION_LIMIT,
            cHttpDaemonConnectionsLimit, MHD_OPTION_PER_IP_CONNECTION_LIMIT, cHttpDaemonConnectionsPerIPLimit,
            MHD_OPTION_LISTENING_ADDRESS_REUSE, cHttpDaemonAddressReuse, MHD_OPTION_HTTPS_MEM_KEY, _keyBuffer.get(),
            MHD_OPTION_HTTPS_MEM_CERT, _certBuffer.get(), MHD_OPTION_END);
    }
...
bool HttpServerImpl::startServerHttp()
{
    ...
    _server = MHD_start_daemon(
        MHD_USE_THREAD_PER_CONNECTION | MHD_USE_INTERNAL_POLLING_THREAD | MHD_USE_DEBUG | MHD_USE_PIPE_FOR_SHUTDOWN,
        _settings.port, nullptr, nullptr, &mhdEventHandler, this, MHD_OPTION_EXTERNAL_LOGGER, mhdLogger, nullptr,
        MHD_OPTION_NOTIFY_COMPLETED, mhdRequestCompleted, nullptr, MHD_OPTION_CONNECTION_LIMIT,
        cHttpDaemonConnectionsLimit, MHD_OPTION_PER_IP_CONNECTION_LIMIT, cHttpDaemonConnectionsPerIPLimit,
        MHD_OPTION_LISTENING_ADDRESS_REUSE, cHttpDaemonAddressReuse, MHD_OPTION_END);
...
int HttpServerImpl::handleHttpRequest(MHD_Connection *mhdConn, HttpConnectionImpl &httpConn)
{
    ...
    if (handle(httpConn) && httpConn.error() == MHD_HTTP_OK)
    {
        if (auto s = httpConn.session().lock())
        {
            ...
            httpConn.setResponseHeader("X-Session-Expiry", time::timestamp_to_string(s->expiration()));
        }
    ...
    if (httpConn == nullptr)
    {
        ...
        else
        {
            ...
            *conCls = httpConn;
            ...
            httpServer->processSessions(*httpConn);
        }
...
void HttpServerImpl::processSessions(HttpConnectionImpl &connection)
{
...
int HttpServerImpl::checkSessionsExpiration()
{
    ...
    _sessions.remove_if([currentTime](HttpSession::WPtr session) {
        ...
        {
            time_t sessionExpiration = s->expiration();
