...
int createPidFile(const char *procname, const char *pidfile)
{
    ...
    softeq::common::system::Path pidpath(pidfile);
    if (pidpath.hasParent())
    {
        softeq::common::system::Path parent_path = pidpath.parent();
        std::error_code ec;
        if (!parent_path.exist() && !softeq::common::system::mkdirs(parent_path))
        {
            LOGE(LOG_DOMAIN, "Failed to create path %s (%s)", std::string(parent_path).c_str(), strerror(errno));
            return -EPERM;
        }
    }
...
void SystemService::handleSignal(int signum)
{
    ...
    case SIGHUP:
        ...
        if (result)
        {
            LOGI(LOG_DOMAIN, "Reconfiguration result: success");
        }
        else
        {
            LOGE(LOG_DOMAIN, "Reconfiguration result: failure");
        }
