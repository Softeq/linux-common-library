...
{
    return Path(::parent(_value));
}

Permissions Path::permissions() const
{
    return getPermissions(_value);
}

bool Path::hasParent() const
{
    return ::hasParent(_value);
}

void Path::updateInternals()
{
    struct stat sb = {};
    if (::lstat(_value.c_str(), &sb) == 0)
    {
        _aTime = sb.st_atime;
        _mTime = sb.st_mtime;
...
void Path::append(const std::string &other)
{
    if (other.empty())
    {
        return;
    ...
    std::string::size_type otherPos = other.find_first_not_of('/');
    std::string::size_type valuePos = _value.find_last_not_of('/');

    // If both parts contains only slashes then leave just one
    if (valuePos == std::string::npos && otherPos == std::string::npos)
    {
        _value = std::string("/");
        return;
    }

    // Crop extra slashes in the end of _value if it is not empty
    if (valuePos != std::string::npos && !_value.empty())
    {
        _value = _value.substr(0, valuePos + 1);
        _value.append("/");
    }

    // Crop extra slashes in the begginning of other part
    std::string appendPart;
    if (otherPos != std::string::npos)
    {
        // But if _value is empty and other part starts with slashes, we need to leave one shash
        // e.g. "" + "///var" = "/var"
        if (_value.empty() && other[0] == '/' && otherPos > 0)
        {
            otherPos -= 1;
        }
        appendPart = other.substr(otherPos);
    }

    // Just append cropped parts
    _value.append(appendPart);
...
std::list<std::string> dirContent(const std::string &base_path, const std::string &filter /*="*"*/)
{
    ...
    softeq::common::stdutils::scope_guard guard;
