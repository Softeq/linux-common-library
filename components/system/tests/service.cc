...
TEST(Service, SysVService)
{
    ...
    if (pid == 0)
    {
        std::string pathToTest{std::string(SVSERVICE_TEST_DIR) + "/" + testFileName};
    ...
    else
    {
        ...
        while(softeq::common::system::exist(pidFile) == false) {
        ...
        ASSERT_FALSE(softeq::common::system::exist(pidFile)) << NAME(pidFile) << " = " << pidFile;
    }
}
