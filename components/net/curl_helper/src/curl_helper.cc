...
void CurlHelper::addRequestParam(const std::string &name, const std::string &value)
{
    if (!_curl_handle || _url.empty() || name.empty() || value.empty())
    {
        return;
    }

    char *encoded = curl_easy_escape(_curl_handle, value.c_str(), static_cast<int>(value.size()));
    if (!encoded)
    {
        return;
    }

    _url.append(_hasParams ? "&" : "?").append(name).append("=").append(encoded);
    _hasParams = true;
    curl_free(encoded);
}
...
void CurlHelper::addRequestHeader(const std::string &name, const std::string &value)
{
    if (_curl_handle)
    {
        std::string header = string_format("%s: %s", name.c_str(), value.c_str());
...
void CurlHelper::deserialize(const std::string &cookie)
{
    ...
    std::vector<std::string> tmp = string_split(data, delimiter);
...
void CurlHelper::serialize(std::string &cookie) const
{
    ...
    {
        cookie += string_format("%s=%s; ", it.first.c_str(), it.second.c_str());
    }
}
...
void CurlHelper::setDefaultCurlHandleOptions()
{
