...
{
    {"common-logging-tests", "debug log test without prefix"},
    {"common-logging-tests", "info log test without prefix"},
    {"common-logging-tests", "System error (Permission denied), "},
};
...
TEST(Logger, SysLog)
{
    ...
    ASSERT_NO_THROW(log().set(LoggerInterface::UPtr(new SystemLogger(name, LOG_PID | LOG_PERROR,
                                                                                                     LOG_USER))));
    ...
    ASSERT_NO_THROW(log().set(LoggerInterface::UPtr(new SystemLogger(name, LOG_PID | LOG_PERROR, 0))));
    LOGI(domain.c_str(), format, "info");
    unsetenv("LOG_PREFIX");
