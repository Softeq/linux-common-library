...
{
    std::string tmp = string_format("Hello world!");
    ASSERT_STREQ(tmp.c_str(), "Hello world!");

    tmp = string_format("%s%s", "1", "2");
    ASSERT_STREQ(tmp.c_str(), "12");

    tmp = string_format("%d%d", 1, 2);
    ASSERT_STREQ(tmp.c_str(), "12");

    tmp = string_format("%d%d%d%d", 1, 2, 3, 4);
    ASSERT_STREQ(tmp.c_str(), "1234");

    tmp = string_format("%02d%02d", 1, 2);
    ASSERT_STREQ(tmp.c_str(), "0102");

    tmp = string_format("%x%03x", 11, 12);
    ASSERT_STREQ(tmp.c_str(), "b00c");

    tmp = string_format("%X%04X", 11, 12);
    ASSERT_STREQ(tmp.c_str(), "B000C");

    tmp = string_format("%s%s", nullptr, nullptr);
...
TEST(StdUtils,StringFormatLongString)
{
    ...
    std::string tmp = string_format("%s%s", longStringA.c_str(), longStringB.c_str());
...
TEST(StdUtils,StringSplit)
{
    ...
    std::vector<std::string> tmp = string_split(path, '/');
